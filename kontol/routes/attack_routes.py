...
def index_flood():
    if 'key' in request.args and 'host' in request.args and 'port' in request.args and 'time' in request.args and 'method' in request.args:
        ...
        return jsonify({"status": "? Not Found", "logs": "Missing argument"}), 404
    
    global attack_slots
    if not all([key, host, port, times, method]):
        return jsonify({"status": "? Not Found", "logs": "Null values."}), 404
    
    if not Validation.is_valid_key(key):
        return jsonify({"status": "? Not Found", "logs": "Key Expired or not valid."}), 404
    ...
    if attack_slots >= max_cons:
        return jsonify({"status": "? Limited conc", "logs": f"Your concurrents max is {max_cons}"}), 429

    if attack_slots > max_slots:
        return jsonify({"status": "? Slot full", "logs": "Slots is full, please wait"}), 429
        
    if method.upper() == "STOP":
        method = method
    else:
        if method.upper() not in layer7_methods and method.upper() not in layer4_methods and method.upper():
            return jsonify({"status": "? Not Found", "logs": "Method invalid."}), 404
        
    if str_validation(port):
        return jsonify({"system": "Error, Malicious character detected."}), 400
    ...
    if not usrdata.get(key):
        return jsonify({"status": "? Not Found", "logs": "Key not valid"}), 404
        
    if is_target_blacklisted(host):
        return jsonify({"status": "? Not Found", "logs": "Target is blacklisted by system."}), 404
        
    if not Validation.validate_ip(host):
        if not Validation.validate_url(host):
            return jsonify({"status": "? Not Found", "logs": "Target is not an Ipv4 or an URL."}), 404
        else:
            host = host
    else:
        host = host
    ...
    if not Validation.validate_port(port):
        return jsonify({"status": "? Not Found", "logs": "Port should be in the range of (1-65535)."}), 404
        
    user_maxtime = db["keys"][key]["maxTime"]
    if not Validation.validate_time(key, times):
        return jsonify({"status": "? Not Found", "logs": f"Time should be MIN=1, MAX={user_maxtime}."}), 404
    ...
    if not is_method_allowed(layer, method):
        return jsonify({"status": "? Not Found", "logs": f"You cannot use VIP plans, methods={method.upper()}"}), 404
    ...
    def process_attack():
        ...
        print(f'Attack completed. Delay: {delay}')
    
    threading.Thread(target=process_attack).start()
